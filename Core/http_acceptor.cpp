...
http_acceptor::http_acceptor(int _epoll_cnt, int _max_conn) {
	...
	//pthread_mutex_init(&wid_mutex, NULL);
}
...
http_acceptor::~http_acceptor() {
	...
	//pthread_mutex_destroy(&wid_mutex);
}
/*
int http_acceptor::open(configuration config) {
...
/*
int http_acceptor::registerHandler(http_handler* _handler) {
	handler = _handler;
	return 0;
}
*/
...
uint32_t http_acceptor::genWid() {
	...
	pthread_spin_lock(&wspinlock);
	tmp = wid;
	wid += 2;
	pthread_spin_unlock(&wspinlock);
